...
class DragonchainClient {
  ...
  createTransaction(String transactionType, dynamic payload,
      {String? tag, String? callbackURL}) async {
    ...
    return await this
        .post('/v1/transaction', transactionBody, callbackURL: callbackURL??"");
  }
  ...
  queryBlocks(String redisearchQuery,
      {bool? verbatim,
      int? offset,
      int? limit,
      String? sortBy,
  ...
  createTransactionType(String transactionType,
      [List<Map<String, String>>? customIndexedFields]) async {
    if (transactionType == null || transactionType == '')
      throw Exception('Empty transaction type');
    var body = {"version": '2', "txn_type": transactionType};
    if (customIndexedFields != null)
      body["customIndexedFields"] =
          this.validateAndBuildCustomIndexFieldsArray(customIndexedFields);
    return await this.post('/v1/transaction-type', body);
  }

  deleteTransactionType(String transactionType) async =>
      await this.delete("/v1/transaction-type/$transactionType");

  listTransactionTypes() async => await this.get("/v1/transaction-types");

  getTransactionType(String transactionType) async =>
      await this.get("/v1/transaction-type/$transactionType");

  getTransaction(String transactionId) async =>
      await this.get("/v1/transaction/$transactionId");
  ...
  updateApiKey(String keyId, String nickname) async =>
      await this.put("/v1/api-key/$keyId", {"nickname": nickname});
  ...
  createSmartContract(String transactionType, String image, String cmd,
    ...
    if (scheduleIntervalInSeconds != null && cronExpression != null)
      throw Exception(
          "Parameters 'scheduleIntervalInSeconds' AND 'cronExpression' are mutually exclusive");
    ...
    if (executionOrder != null &&
        ["parallel", "serial"].contains(executionOrder))
    ...
    if (scheduleIntervalInSeconds != null)
      body["seconds"] = scheduleIntervalInSeconds;
  ...
  getSmartContract({String? smartContractId, String ?transactionType}) async {
    if (smartContractId != null && transactionType != null)
      throw Exception(
          "Only one of 'smartContractId' or 'transactionType' can be specified");
    if (smartContractId != null)
      return await this.get("/v1/contract/$smartContractId");
    if (transactionType != null)
      return await this.get("/v1/contract/txn_type/$transactionType");
    throw Exception(
        "At least one of 'smartContractId' or 'transactionType' must be specified");
  }

  deleteSmartContract(String smartContractId) async =>
      await this.delete("/v1/contract/$smartContractId");
  ...
  getSmartContractLogs(String smartContractId, {int? tail, String? since}) async {
    ...
    return await this.get(
        "/v1/contract/$smartContractId/logs${this.generateQueryString(queryParams)}");
  }

  listSmartContracts() async => await this.get("/v1/contract");

  getSmartContractObject(String key, String smartContractId) async =>
      this.get("/v1/get/$smartContractId/$key", false);
  ...
  listSmartContractObjects(String smartContractId, [String? prefixKey]) async {
    ...
    if (prefixKey != null) {
      if (prefixKey.endsWith("/"))
        throw Exception("Parameter 'prefixKey' cannot end with '/'");
      path += "$prefixKey/";
    }
    return await this.get(path);
  }

  getPendingVerifications(String blockId) async =>
      await this.get("/v1/verifications/pending/$blockId");

  getVerifications(String blockId, [int? level]) async {
    if (level != null)
      return await this.get("/v1/verifications/$blockId?level=$level");
  ...
  validateAndBuildCustomIndexFieldsArray(
      List<Map<String, dynamic>> customIndexedFields) {
    ...
    customIndexedFields.forEach((customIndexedField) {
      ...
      if (customIndexedField["options"] != null) {
        ...
        if (customIndexedField["options"]["noIndex"] != null)
          optionsBody["no_index"] = customIndexedField["options"]["noIndex"];
        if (customIndexedField["type"] == "tag") {
          if (customIndexedField["options"]["separator"] != null)
            optionsBody["separator"] =
                customIndexedField["options"]["separator"];
        } else if (customIndexedField["type"] == "text") {
          if (customIndexedField["options"]["noStem"] != null)
            optionsBody["no_stem"] = customIndexedField["options"]["noStem"];
          if (customIndexedField["options"]["weight"] != null)
            optionsBody["weight"] = customIndexedField["options"]["weight"];
          if (customIndexedField["options"]["sortable"] != null)
            optionsBody["sortable"] = customIndexedField["options"]["sortable"];
        } else if (customIndexedField["type"] == "number") {
          if (customIndexedField["options"]["sortable"] != null)
            optionsBody["sortable"] = customIndexedField["options"]["sortable"];
        } else {
          throw Exception(
              "Parameter 'customIndexedFields[].type' must be 'tag', 'text' or 'number'");
        }
  ...
  getHttpHeaders(String path, String method,
    ...
    Map<String, String> headers = {
      ...
      "authorization": this
          .credentialService
          .getAuthorizationHeader(method, path, timestamp, contentType, body),
      "timestamp": timestamp
    };
  ...
  makeRequest(String path, String method,
      {String body = '', bool parse = true}) async {
    String contentType = '';
    ...
    return (responseBody != null && method != 'DELETE' || parse == true)
  ...
  static createClient(
    ...
    if (dragonchainId == null || dragonchainId == '')
      throw Exception('Did not provide dragonchain ID');
    if (endpoint == null || endpoint == '')
      endpoint = await ConfigService.getDragonchainEndpoint(dragonchainId);
    logger.d(endpoint);
    var credentials = new CredentialService(
        dragonchainId, {"authKeyId": authKeyId??'', "authKey": authKey??""}, algorithm);
